 { config, pkgs, ... }:
{
 programs.zsh = {
    ...
    shellAliases = {
      c = "clear && fastfetch";
      "nix-update" = "cd ~/.nix-config/nix && sudo nix flake update";
      "nix-rebuild" = "cd ~/.nix-config/nix && sudo nixos-rebuild switch --flake";
      ...
      "git-update" = "cd ~/.nix-config && git commit . -meow && git push -u origin main";
